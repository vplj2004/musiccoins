      ...
      <div className="w-full max-w-md backdrop-blur-xl bg-white/10 rounded-xl p-6 border border-white/10">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input id="username" className="w-full px-4 py-2 rounded bg-black/40 border border-white/10" placeholder="dj_sonic" value={username} onChange={(e) => setUsername(e.target.value)} />
          </div>
          <div>
        ...
        <p className="mt-4 text-sm text-white/70">Already have an account? <Link href="/login" className="underline">Log in</Link></p>
      </div>
    </div>
  );
}
